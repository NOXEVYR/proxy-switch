...
自动化结果见 [本轮测试记录](TEST_REPORT.md)。实际目标电脑还需完成：

- 先观察启动器、游戏主程序与登录子进程，确认已保存线路与实际入口/出口分别显示；未确认连接保持未知。
- 预览整组规则时显示遗漏、已覆盖、保留冲突与身份未知；确认仅补缺，网站例外保留。
- 正常退出整组程序后测试干净启动；必要时明确进行限时直连对照，记录是否进入大厅。恢复后继续验证，不把成功传输当作登录。
- 对照会临时影响当前用户系统代理；到期或提前结束后检查恢复结果。外部修改保留，恢复失败须处理，不以日志缺失推断未启动。
- 正常退出旧版后从新版桌面入口启动，确认双入口任务栏图标；旧固定入口可重新固定。原目录和配置备份保留。
